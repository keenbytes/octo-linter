...
// NotOneWord_InWorkflow checks for variable references that are single-word or single-level, e.g. `${{ something }}` instead of `${{ inputs.something }}`.
// Only the values `true` and `false` are permitted in this form; all other variables are considered invalid.
type NotOneWord_InWorkflow struct {
}

func (r NotOneWord_InWorkflow) ConfigName(int) string {
	return "referenced_variables_in_workflows__not_one_word"
}

func (r NotOneWord_InWorkflow) FileType() int {
	return rule.DotGithubFileTypeWorkflow
}

func (r NotOneWord_InWorkflow) Validate(conf interface{}) error {
	_, ok := conf.(bool)
	if !ok {
...
func (r NotOneWord_InWorkflow) Lint(conf interface{}, f dotgithub.File, d *dotgithub.DotGithub, chErrors chan<- string) (compliant bool, err error) {
	compliant = true
