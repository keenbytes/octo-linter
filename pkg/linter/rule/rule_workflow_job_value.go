	...
	for jobName := range w.Jobs {
		for k, v := range r.Value {
			if k == "name" && v != "" {
				...
				if !m {
					compliant = false
					printErrOrWarn(r.ConfigName, r.IsError[k], r.LogLevel, fmt.Sprintf("workflow '%s' job '%s' name must be lower-case and hyphens only", w.FileName, jobName), chWarnings, chErrors)
				}
			}
		}
	}
