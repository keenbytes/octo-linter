	...
	for _, v := range varTypes {
		...
		for _, f := range found {
			if v == "vars" && len(d.Vars) > 0 && !d.IsVarExist(string(f[1])) {
				printErrOrWarn(r.ConfigName, r.IsError, r.LogLevel, fmt.Sprintf("workflow '%s' calls a variable '%s' that does not exist in the vars file", w.FileName, string(f[1])), chWarnings, chErrors)
				compliant = false
			}

			if v == "secrets" && len(d.Secrets) > 0 && !d.IsSecretExist(string(f[1])) {
				printErrOrWarn(r.ConfigName, r.IsError, r.LogLevel, fmt.Sprintf("workflow '%s' calls a secret '%s' that does not exist in the secrets file", w.FileName, string(f[1])), chWarnings, chErrors)
				compliant = false
			}
		}
	}
